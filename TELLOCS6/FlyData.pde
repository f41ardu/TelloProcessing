  ...
  public class FlightData
        {
         ...
         public void set(byte[] data)
            {
                int index = 9;
                qheight = (int)((data[index] & 0xff ) | (data[index + 1] << 8)); index += 2;
                northSpeed = (int)((data[index] & 0xff )| (data[index + 1] << 8)); index += 2;
                eastSpeed = (int)((data[index] & 0xff ) | (data[index + 1] << 8)); index += 2;
                flySpeed = ((int)Math.sqrt(Math.pow(northSpeed, 2.0) + Math.pow(eastSpeed, 2.0)));
                verticalSpeed = (int)((data[index] & 0xff )| (data[index + 1] << 8)); index += 2;// ah.a(paramArrayOfByte[6], paramArrayOfByte[7]);
                ...
                droneFlyTimeLeft = data[index] & 0xff| (data[index + 1] << 8); index += 2;
                droneBatteryLeft = data[index] & 0xff | (data[index + 1] << 8); index += 2;
